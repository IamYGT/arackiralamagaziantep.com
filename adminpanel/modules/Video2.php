...
class Video2 extends Settings  {

    public $SayfaBaslik = 'Video Listesi';
    public  $modulName = 'Video2';
    public  $icbaslik ;
    private $js = array();
    private $css = array();
    private $siteURL;
    private $set;
    private $module = "videolar";
    ...
    public function ekle($id=null)
    {
        ...
        if(isset($id) and $id) $data = $this->dbConn->tekSorgu('select * from videolar2 WHERE id='.$id);
        ...
        //$text .= $form->file(array('url'=>$this->BaseURL().'upload','title'=>'Resim','name'=>'VideoGenelResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('haber'),'src'=>((isset($data['resim'])) ? $data['resim'] :'')));
        $text .= $form->file(array('url'=>$this->BaseURL("upload").'/videolar2','folder'=>'videolar2','title'=>'Resim','name'=>'VideoGenelResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('haber'),'src'=>((isset($data['resim'])) ? $data['resim'] :'')));
    ...
    public function kaydet($id=null)
    {
        ...
        if(isset($id) and $id):
            //Güncelle
            $this->dbConn->update('videolar2',$post,$id);

        else:
            // kaydet
            $this->dbConn->insert('videolar2',$post,$id);
    ...
    {
        if($id) $this->dbConn->sil("DELETE FROM videolar2 where masterid='$id' or id=".$id);
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/liste.html'));
    }
    ...
    public function liste($id=null)
    {
        ...
        return $pagelist->Pagelist(array(
            ...
            'pdata' => $this->dbConn->sorgu('select * from videolar2 where masterid=0 order by  sira'),
            'baslik'=>array(
