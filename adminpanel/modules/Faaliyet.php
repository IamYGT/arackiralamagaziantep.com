...
class Faaliyet extends Settings  {

    public  $SayfaBaslik = 'Faaliyet';
    public  $modulName = 'Faaliyet';
    public  $icbaslik ;
    private $js = array();
    private $css = array();
    private $module='faaliyet';
    private $table ='faaliyet';
    private $tablelang = 'faaliyet_lang';
    ...
    public function ekle($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text'] .= $form->textarea(array('value'=>((isset($data[$dil]['ozet']) ? $this->temizle($data[$dil]['ozet']) :'')),'title'=>'Özet','name'=>'ozet','id'=>'ozet','help'=>'Özeti buraya girebilirsiniz.','height'=>'120','lang'=>$dil));
            $tabForm[$dil]['text'] .= $form->textEditor(array('value'=>((isset($data[$dil]['detay']) ? $this->temizle($data[$dil]['detay']) :'')),'title'=>'Detayı','name'=>'detay','id'=>'haberDetay','height' => '250','lang'=>$dil));
            $tabForm[$dil]['text'] .= $form->input(array('value'=>((isset($data[$dil]['icon']) ? $this->temizle($data[$dil]['icon']) :'')),'title'=>'icon','name'=>'icon','id'=>'icon','lang'=>$dil));
            ...
            foreach ($this->settings->lang('lang') as $dil=>$title):
              if($dil == "tr"):
                    $post[$dil] = array(
                        ...
                        'tarih' => strtotime($this->_POST('tarih_tr')),
                        'detay'=>$this->kirlet($this->_POST('detay',$dil)),
                        'icon'=>$this->kirlet($this->_POST('icon',$dil)),
                    ...
                    $post[$dil] = array(
                        ...
                        'tarih' => strtotime($this->_POST('tarih_tr')),
                        'detay'=>$this->kirlet($this->_POST('detay',$dil)),
                        'url'=> $this->SeoURL($this->table.'_lang',array('name'=>'baslik','value'=>$this->kirlet($this->_POST('baslik',$dil)),'id'=>$id)),
                        'ozet'=>$this->kirlet($this->_POST('ozet',$dil)),
                        'dil'=>$dil
                    );
