...
class Urunfoto extends Settings {
    ...
    public $SayfaBaslik = 'Ürün Resimleri';
    ...
    public function urunResimduzenle($id=null)
    {
        ...
        $text .= $form->file(array('url'=>$this->BaseURL('upload'),'folder'=>'','title'=>'Resim','name'=>'fotoResim','resimBoyut'=>$this->settings->boyut('icerik'),'src'=>((isset($urun['dosya'])) ? $urun['dosya'] :'')));
    ...
    {
        $modul = "3";
        if(isset($id) and $id) $urun = $this->dbConn->tekSorgu('select * from urunler  WHERE id='.$id);

        $this->icbaslik = $urun['baslik']." Ürününe Resim Ekle";
        ...
        return $pagelist->Fotolist(array(
            ...
            'yukle'=> array('type'=>'button','title'=>'Resim Ekle','class'=>'btn btn-danger','modul'=>$modul,'folder'=>'../'.$this->settings->config('folder'),'name'=>((isset($urun['url'])) ? $urun['url']:null)),

            'buton'=> array(
            //   array('type'=>'radio','dataname'=>'durum','url'=>$this->BaseAdminURL($this->modulName.'/aktif/'),'item'=>array('aktif'=> 'Arka','pasif'=> 'Ön')),
            ),
            'pdata' => $this->dbConn->sorgu("select * from dosyalar where type = ".$modul." and data_id='$id' ORDER   BY  sira"),
            'baslik'=>array(
                array('title'=>'ID','width'=>'4%'),
    ...
    public function KPlanEkle($id=0)
    {
        $modul = "10";

        if(isset($id) and $id) $urun = $this->dbConn->tekSorgu('select * from urunler WHERE id='.$id);

        $this->icbaslik = $urun['baslik']." Projesine Kat Planı Ekle";


        $pagelist = new Pagelist($this->settings);

        return $pagelist->Fotolist(array(
            'title'=> 'Kat Planı Listesi',
            'icbaslik' => $this->icbaslik,
            'id'=>$id,
            'page'=>'dosyalar',
            //    'button' => array(array('title'=>'Fotograf Ekle','href'=> $this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'green')),
            ...
            'tools' =>array(
                array('title'=>'Düzenle','icon'=>'fa fa-times','url'=>$this->BaseAdminURL($this->modulName.'/KplanDuzenle/'),'color'=>'blue'),
                array('title'=>'Sil','icon'=>'fa fa-edit','url'=>$this->BaseAdminURL($this->modulName.'/KPlanSil/'),'color'=>'red')),
            'yukle'=> array('type'=>'button','title'=>'Kat Planı Ekle','class'=>'btn btn-danger','modul'=>$modul,'folder'=>'../'.$this->settings->config('folder'),'name'=>((isset($urun['url'])) ? $urun['url']:null)),
      ...
      public function KplanDuzenle($id=null)
    {
        $this->icbaslik = 'Kat Planı Ekle';
        if(isset($id) and $id) $urun = $this->dbConn->tekSorgu('select * from dosyalar  WHERE id='.$id);
        $text = '';

        $form = new Form($this->settings);
        $text .= $form->formOpen(array('method'=>'POST','icbaslik'=>$this->icbaslik,'action'=>  $this->BaseAdminURL($this->modulName.'/KplanKaydet'.((isset($id)) ? '/'.$id :'')),'fileUpload'=>true,'id'=>'form_sample_3','class'=>''));
        $text .= $form->input(array('value'=>((isset($urun['baslik']) ? $urun['baslik'] :'')),'title'=>' Başlık','name'=>'baslik','id'=>'baslik','help'=>''));
        //   $text .= $form->textarea(array('value'=>((isset($yazi['detay']) ? $yazi['detay'] :'')),'title'=>'Açıklama','name'=>'ozet','id'=>'ozet','help'=>'','height'=>'120'));
        $text .= $form->file(array('url'=>$this->BaseURL('upload'),'folder'=>'','title'=>'Resim','name'=>'fotoResim','resimBoyut'=>$this->settings->boyut('icerik'),'src'=>((isset($urun['dosya'])) ? $urun['dosya'] :'')));
...
public  function KplanKaydet($id=null)
    {
        ...
        $urun = $this->dbConn->teksorgu("select * from dosyalar where id='$id' and type = ".$modul);
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/KPlanEkle/'.$urun['data_id']));


    }

     public function KPlanSil($id=null)
    {
        $rec2 = $this->dbConn->tekSorgu("SELECT * FROM dosyalar WHERE id='$id'");

        $resim2= $this->resimGet($rec2["dosya"]);
        $kid = $rec2['data_id'];
        $this->ResimSil($resim2,"../upload/"); // Eski resmi sil
        if($id) $this->dbConn->sil('DELETE FROM dosyalar where id='.$id);
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/KPlanEkle/'.$kid));
    }



    public function DPlanEkle($id=0)
    {
        $modul = "11";

        if(isset($id) and $id) $urun = $this->dbConn->tekSorgu('select * from urunler WHERE id='.$id);

        $this->icbaslik = $urun['baslik']." Projesine Daire Planı Ekle";


        $pagelist = new Pagelist($this->settings);

        return $pagelist->Fotolist(array(
            'title'=> 'Daire Planı Listesi',
            'icbaslik' => $this->icbaslik,
            'id'=>$id,
            'page'=>'dosyalar',
            //    'button' => array(array('title'=>'Fotograf Ekle','href'=> $this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'green')),
            ...
            'tools' =>array(
                array('title'=>'Düzenle','icon'=>'fa fa-times','url'=>$this->BaseAdminURL($this->modulName.'/DplanDuzenle/'),'color'=>'blue'),
                array('title'=>'Sil','icon'=>'fa fa-edit','url'=>$this->BaseAdminURL($this->modulName.'/DPlanSil/'),'color'=>'red')),
            'yukle'=> array('type'=>'button','title'=>'Daire Planı Ekle','class'=>'btn btn-danger','modul'=>$modul,'folder'=>'../'.$this->settings->config('folder'),'name'=>((isset($urun['url'])) ? $urun['url']:null)),
     ...
     public function DplanDuzenle($id=null)
    {
        $this->icbaslik = 'Daire Planı Ekle';
        if(isset($id) and $id) $urun = $this->dbConn->tekSorgu('select * from dosyalar  WHERE id='.$id);
        $text = '';

        $form = new Form($this->settings);
        $text .= $form->formOpen(array('method'=>'POST','icbaslik'=>$this->icbaslik,'action'=>  $this->BaseAdminURL($this->modulName.'/DplanKaydet'.((isset($id)) ? '/'.$id :'')),'fileUpload'=>true,'id'=>'form_sample_3','class'=>''));
        $text .= $form->input(array('value'=>((isset($urun['baslik']) ? $urun['baslik'] :'')),'title'=>' Başlık','name'=>'baslik','id'=>'baslik','help'=>''));
        //   $text .= $form->textarea(array('value'=>((isset($yazi['detay']) ? $yazi['detay'] :'')),'title'=>'Açıklama','name'=>'ozet','id'=>'ozet','help'=>'','height'=>'120'));
        $text .= $form->file(array('url'=>$this->BaseURL('upload'),'folder'=>'','title'=>'Resim','name'=>'fotoResim','resimBoyut'=>$this->settings->boyut('icerik'),'src'=>((isset($urun['dosya'])) ? $urun['dosya'] :'')));
        ...
        public  function DplanKaydet($id=null)
        {
            ...
            $urun = $this->dbConn->teksorgu("select * from dosyalar where id='$id' and type = ".$modul);
            $this->RedirectURL($this->BaseAdminURL($this->modulName.'/DPlanEkle/'.$urun['data_id']));


        }

           public function DPlanSil($id=null)
            {
                $rec2 = $this->dbConn->tekSorgu("SELECT * FROM dosyalar WHERE id='$id'");
                ...
                $this->ResimSil($resim2,"../upload/"); // Eski resmi sil
                if($id) $this->dbConn->sil('DELETE FROM dosyalar where id='.$id);
                $this->RedirectURL($this->BaseAdminURL($this->modulName.'/DPlanEkle/'.$kid));
            }
    ...
    public  function urunResimkaydet($id=null)
    {
        ...
        $urun = $this->dbConn->teksorgu("select * from dosyalar where id='$id' and type = ".$modul);
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/urunResimEkle/'.$urun['data_id']));
    ...
    public function urunResimSil($id=null)
    {
        ...
        $this->ResimSil($resim2,"../upload/"); // Eski resmi sil
        if($id) $this->dbConn->sil('DELETE FROM dosyalar where id='.$id);
        $this->RedirectURL($this->BaseAdminURL($this->modulName.'/urunResimEkle/'.$kid));
    }
} 