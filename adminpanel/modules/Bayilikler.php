...
class Bayilikler extends Settings  {

    public $SayfaBaslik = 'Bayilikler';
    public  $modulName = 'Bayilikler';
    public $icbaslik ;
    private $js = array();
    private $css = array();
    private $module = 3;
    private $table = "bayilikler";
    private $tablelang = "bayilikler_lang";
    ...
    public function ekle($id=null)
    {
       ...
       $this->icbaslik = 'Bayilik Ekle';
        ...
        $text.= $form->input(array('value'=>((isset($data["tr"]['firma_url']) ? $this->temizle($data["tr"]['firma_url']) :'')),'title'=>'Web Adresi','name'=>'firma_url','id'=>'firma_url','help'=>'Varsa Firma Web Adresini Buraya girebilirsiniz'));


        $text .= $form->file(array('url'=>$this->BaseURL().'upload','title'=>'Firma Logosu','name'=>'refResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('bayilik'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'firma_url'=> $this->_POST('firma_url'),
                    'dil'=>$dil,
    ...
    {
       if($id) $this->dbConn->sil('DELETE FROM bayilikler where id='.$id);
       if($id) $this->dbConn->sil('DELETE FROM bayilikler_lang where master_id='.$id);
       $this->RedirectURL($this->BaseAdminURL($this->modulName.'/liste.html'));
    }
    ...
    public function liste($id=null)
    {
      ...
      return $pagelist->Pagelist(array(
         'title'=> 'Bayilik Listesi',
          'page'=>'Bayilik',
         'button' => array(array('title'=>'Bayilik Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'green')),
         'p'=>array(
         ...
         'pdata' => $this->dbConn->sorgu('select * from bayilikler ORDER   BY  sira,id'),
         'baslik'=>array(
     ...
     public function vitrin($id=null)
    {
        ...
        $bayiDuzenle = $this->dbConn->update('bayilikler',array('durum'=>$durum),$id);
        $langDuzenle = $this->dbConn->langUpdate('bayilikler_lang',array('durum'=>$durum),$id);
