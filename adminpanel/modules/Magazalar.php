...
class Magazalar extends Settings  {

    public  $SayfaBaslik = 'Referanslar';
    public  $modulName = 'magazalar';
    public  $icbaslik ;
    private $js = array();
    private $css = array();
    private $module = 3;
    private $table = "magazalar";
    private $tablelang = "magazalar_lang";
    ...
    public function ekle($id=null)
    {
       ...
       $this->icbaslik = 'Referans Ekle';
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['sehir']) ? $this->temizle($data[$dil]['sehir']) :'')),'title'=>'Şehir','name'=>'sehir','id'=>'sehir','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['guc']) ? $this->temizle($data[$dil]['guc']) :'')),'title'=>'Güç','name'=>'guc','id'=>'guc','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['isi']) ? $this->temizle($data[$dil]['isi']) :'')),'title'=>'İş','name'=>'isi','id'=>'isi','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['miktar']) ? $this->temizle($data[$dil]['miktar']) :'')),'title'=>'Miktar','name'=>'miktar','id'=>'miktar','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['yil']) ? $this->temizle($data[$dil]['yil']) :'')),'title'=>'Yılı','name'=>'yil','id'=>'yil','help'=>'','lang'=>$dil));

        endforeach;
        $text .= $tabs->tabContent($tabForm);
        //$text .= $form->file(array('url'=>$this->BaseURL("upload").'/magazalar','folder'=>'magazalar','title'=>'Resim','name'=>'refResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('refResim'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'sehir'=> $this->kirlet($this->_POST('sehir',$dil)),
                    'guc'=> $this->kirlet($this->_POST('guc',$dil)),
                    'isi'=> $this->kirlet($this->_POST('isi',$dil)),
                    'miktar'=> $this->kirlet($this->_POST('miktar',$dil)),
                    'yil'=> $this->kirlet($this->_POST('yil',$dil)),
                    'dil'=>$dil,
    ...
    public function liste($id=null)
    {
        ...
        if (isset($_GET["kelime"])){
            $ek = "WHERE baslik LIKE '%".$_GET["kelime"]."%'";
        }
