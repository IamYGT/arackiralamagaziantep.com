...
class Fiyatlar extends Settings  {

    public $SayfaBaslik = 'Fiyatlar';
    public  $modulName = 'Fiyatlar';
    public $icbaslik ;
    private $js = array();
    private $css = array();
    private $siteURL;
    private $set;
    private $module = 'fiyatlar';
    private $table = 'fiyatlar';
    private $tablelang = 'fiyatlar_lang';
    ...
    public function ekle($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            $tabForm[$dil]['text']  = $form->input(array('value'=>((isset($data[$dil]['ozet']) ? $this->temizle($data[$dil]['ozet']) :'')),'title'=>'Beden Value','name'=>'ozet','id'=>'ozet','help'=>' ( XS ) bu Şekilde Yazısınız. ','lang'=>$dil));
        ...
        //$text .= $form->file(array('url'=>$this->BaseURL("upload").'/fiyatlar','folder'=>'fiyatlar','title'=>'Resim','name'=>'refResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('refResim'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
