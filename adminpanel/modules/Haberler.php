...
class Haberler extends Settings  {

    public $SayfaBaslik = 'Haberler';
    public  $modulName = 'Haberler';
    public $icbaslik ;
    private $js = array();
    private $css = array();
    private $siteURL;
    private $set;
    private $module = 'haberler';
    private $table = 'haberler';
    private $tablelang = 'haberler_lang';
    ...
    public function ekle($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            //$tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['tarih']) ? $this->temizle($data[$dil]['tarih']) :'')),'title'=>'Tarih','name'=>'tarih','id'=>'tarih','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->textarea(array('value'=>((isset($data[$dil]['ozet']) ? $this->temizle($data[$dil]['ozet']) :'')),'title'=>'Özet','name'=>'ozet','id'=>'ozet','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->textEditor(array('value'=>((isset($data[$dil]['detay']) ? $this->temizle($data[$dil]['detay']) :'')),'title'=>'Detay','name'=>'detay','id'=>'detay','help'=>'','lang'=>$dil));
        endforeach;
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'tarih'=> $this->kirlet($this->_POST('tarih',$dil)),
