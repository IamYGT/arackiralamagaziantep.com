...
class Edergi extends Settings  {

    public $SayfaBaslik = 'E-Dergi';
    public $modulName = 'Edergi';
    public $icbaslik ;

    private $module = 'dergi';
    private $table = 'dergi';
    private $tablelang = 'dergi_lang';
    ...
    public function ekle($id=null)
    {
       ...
       if(isset($id) and $id) $data = $this->dbConn->tekSorgu('select * from '.$this->module.' WHERE id='.$id);
        ...
        $text = $form->formOpen(array('method'=>'POST','action'=> $this->BaseAdminURL($this->modulName.'/kaydet'.((isset($id)) ? '/'.$id :'')),'fileUpload'=>true,'id'=>'form_sample_3','class'=>'','icbaslik'=>$this->icbaslik));
            ...
            $text.= $form->input(array('value'=>((isset($data['boyut']) ? $this->temizle($data['boyut']) :'')),'title'=>'Dergi Boyutu (GenxYük)','name'=>'boyut','id'=>'baslik','help'=>'Örneğin (900x1200)','lang'=>"tr"));
    ...
    public function sil($id=null)
    {
        if($id){
            ...
            $this->dbConn->sil("DELETE FROM dosyalar where type='dergi' and data_id=".$id);
    ...
    public function liste($id=null)
    {
       $pagelist = new Pagelist($this->settings);
        ...
        return $pagelist->Pagelist(array(
            'title'=> 'E-Dergi (Önerilen Resim Boyutu : '.$data["boyut"].")",
            'page'=>'dergi',
            'p'=>array(
    ...
    public function KatalogResimEkle($id=null)
    {
        ...
        $this->icbaslik = 'Dergi Sayfa Ekle <br> <span style="font-size:14px;">Yeni Dergi Ekliyorsanız Önce <a href="'.$this->BaseAdminURL($this->modulName.'/silTumu/').'" style="color:#f00">Buraya Tıklayarak</a> Önceki Sayfaları Siliniz.</span>';
        ...
        return $pagelist->Fotolist(array(
            ...
            'yukle'=> array('type'=>'button','title'=>'Resim Ekle','class'=>'btn btn-primary','modul'=>$this->module,'folder'=>'../'.$this->settings->config('folder').$this->module."/",'name'=>$this->permalink($data["baslik"]), 'baslik'=>$this->permalink($data["baslik"])),
    ...
    public function KatalogResimSil($id=null)
    {
        if($id) {
            ...
            $sil = $_SERVER['DOCUMENT_ROOT']."/upload/dergi/".$dosya;
    ...
    public function silTumu($id=null)
    {
            ...
            $data = $this->dbConn->sorgu("select * from dosyalar where type='dergi'");

            foreach ($data as $veri) {
                $dosya = $_SERVER['DOCUMENT_ROOT']."/upload/dergi/".$veri["dosya"];
               
                @unlink($dosya);

                @$this->_KLASORSIL($_SERVER['DOCUMENT_ROOT']."upload/dergi/temp");
            }

            @$this->dbConn->sil("DELETE FROM  dosyalar where type='dergi'");
