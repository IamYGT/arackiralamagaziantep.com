...
class Iletisim extends Settings  {

    public $SayfaBaslik = 'İletişim';
    public  $modulName = 'Iletisim';
    public $icbaslik ;
    private $js = array();
    private $css = array();
    private $siteURL;
    private $set;
    private $module = 'iletisim';
    private $table = 'iletisim';
    private $tablelang = 'iletisim_lang';
    ...
    public function ekle($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text']  = $form->input(array('value'=>((isset($data[$dil]['baslik']) ? $this->temizle($data[$dil]['baslik']) :'')),'title'=>'Başlık','name'=>'baslik','id'=>'baslik','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['baslik2']) ? $this->temizle($data[$dil]['baslik2']) :'')),'title'=>'Alt Başlık','name'=>'baslik2','id'=>'baslik2','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['baslik3']) ? $this->temizle($data[$dil]['baslik3']) :'')),'title'=>'Adres','name'=>'baslik3','id'=>'baslik3','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['ozet']) ? $this->temizle($data[$dil]['ozet']) :'')),'title'=>'Telefon','name'=>'ozet','id'=>'ozet','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['detay']) ? $this->temizle($data[$dil]['detay']) :'')),'title'=>'E-mail','name'=>'detay','id'=>'detay','help'=>'','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->textarea(array('value'=>((isset($data[$dil]['embed']) ? $this->temizle($data[$dil]['embed']) :'')),'title'=>'Harita Iframe','name'=>'embed','id'=>'embed','help'=>'','lang'=>$dil));
        endforeach;

        $text .= $tabs->tabContent($tabForm);
        //$text .= $form->file(array('url'=>$this->BaseURL("upload").'/haber','folder'=>'haber','title'=>'Resim','name'=>'refResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('refResim'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'baslik3'=> $this->kirlet($this->_POST('baslik3',$dil)),
    ...
    public function liste($id=null)
    {
        ...
        return $pagelist->Pagelist(array(
            'title'=> 'İletişim Bilgileri Listesi',
            'page'=>'iletisim',
            'button' => array(array('title'=>'Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'red')),
            'p'=>array(
            ...
            'buton'=> array(
                ...
                //array('type'=>'button2','title'=>'İç Resim Ekle','class'=>'btn btn-primary','dataname'=>'fotoekle','url'=>$this->BaseAdminURL($this->modulName.'/fotoekle/'))
            ),
