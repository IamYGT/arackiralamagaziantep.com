...
class Faydali extends Settings  {

    public $SayfaBaslik = 'Faydalı Bilgiler';
    public $modulName = 'Faydali';
    public $icbaslik ;
    private $js = array();
    private $css = array();
    private $module = 'faydali';
    private $table = 'faydali';
    private $tablelang = 'faydali_lang';
    ...
    public function ekle($id=null)
    {
        ...
        $this->icbaslik = 'Faydalı Bilgi Ekle';
        $tabForm = array();
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            $tabForm[$dil]['text']  = $form->input(array('value'=>((isset($data[$dil]['baslik']) ? $data[$dil]['baslik'] :'')),'title'=>'Başlık','name'=>'baslik','id'=>'baslik','help'=>'','lang'=>$dil));
        ...
        $text .= $form->file(array('url'=>$this->BaseURL().'upload','title'=>'Faydali Bilgi Resmi','name'=>'FaydaliResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('haber'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'resim' => $this->_RESIM('FaydaliResim_tr')
                );
            ...
            foreach ($this->settings->lang('lang') as $dil=>$title):
                if($dil!='tr') {
                    if(count($this->dbConn->sorgu("select * from ".$this->tablelang." where dil='".$dil."' and master_id='".$id."' "))==1)
                        ...
                        $this->dbConn->insert($this->tablelang.'_lang',array_merge($post[$dil],array('master_id'=>$id)));
                }
            endforeach;
        else:
            // kaydet
    ...
    public function liste($id=null)
    {
        ...
        return $pagelist->Pagelist(array(
            'title'=> 'Faydalı Bilgi Listesi',
            'page'=>'faydali',
            'button' => array(array('title'=>'Faydalı Bilgi Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'green')),
            'p'=>array(
