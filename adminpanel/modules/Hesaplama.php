...
class Hesaplama extends Settings  {

    public  $SayfaBaslik = 'Hesaplama';
    public  $modulName = 'hesaplama';
    public  $icbaslik ;
    private $js = array();
    private $css = array();
    private $module = 3;
    private $table = "hesaplama";
    private $tablelang = "hesaplama_lang";
    ...
    public function ekle($id=null)
    {
       ...
       $this->icbaslik = 'Hesaplama Ekle';
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text']  = $form->input(array('value'=>((isset($data[$dil]['firma']) ? $this->temizle($data[$dil]['firma']) :'')),'title'=>'Komisyon','name'=>'firma','id'=>'firma','help'=>'Komisyon','lang'=>$dil));
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['yazi']) ? $this->temizle($data[$dil]['yazi']) :'')),'title'=>'Sabit Bilgilendirme Yazısı','name'=>'yazi','id'=>'firma','help'=>'Sabir Bilgilendirme Yazisi','lang'=>$dil));

        endforeach;

        $text .= $tabs->tabContent($tabForm);

        $text .= $form->input(array('value'=>((isset($data["tr"]['sehir'])) ? $this->temizle($data["tr"]['sehir']) :''),'title'=>'Yüzde','name'=>'sehir','id'=>'sehir','help'=>'Hesaplanacak Olan Yüzde Miktarını (TL) Bazlı Yazınız','lang'=>"tr"));

        //$text .= $form->file(array('url'=>$this->BaseURL("upload").'/hesaplama','folder'=>'hesaplama','title'=>'Resim','name'=>'refResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('refResim'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'yazi'=> $this->kirlet($this->_POST('yazi',$dil)),
    ...
    public function liste($id=null)
    {
      ...
      return $pagelist->Pagelist(array(
         ...
         'page'=>$this->modulName,
         'button' => array(array('title'=>'Hesaplama Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'green')),
         'p'=>array(
         ...
         'tools' =>array(array('title'=>'Düzenle','icon'=>'fa fa-times','url'=>$this->BaseAdminURL($this->modulName.'/ekle/'),'color'=>'blue'),
          ...
          'buton'=> array(
              //array('type'=>'radio','dataname'=>'durum','url'=>$this->BaseAdminURL($this->modulName.'/vitrin/')),
          ),
