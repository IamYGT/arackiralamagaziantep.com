...
class Projeler extends Settings  {

    public  $SayfaBaslik = 'Projeler';
    public  $modulName = 'Projeler';
    public  $icbaslik ;
    private $js = array();
    private $css = array();
    private $module = 3;
    private $table = "projeler";
    private $tablelang = "projeler_lang";
    ...
    public function ekle($id=null)
    {
       ...
       $this->icbaslik = 'Proje Ekle';
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text']  = $form->input(array('value'=>((isset($data[$dil]['firma']) ? $this->temizle($data[$dil]['firma']) :'')),'title'=>'Başlık','name'=>'firma','id'=>'firma','help'=>'Proje Başlığını buraya girebilirsiniz.','lang'=>$dil));
        ...
        /*
        $text .= $form->file(array('url'=>$this->BaseURL("upload").'/projeler','folder'=>'projeler','title'=>'Resim','name'=>'refResim','lang'=>'tr','resimBoyut'=>$this->settings->boyut('refResim'),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        */
