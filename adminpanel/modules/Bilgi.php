...
class Bilgi extends Settings  {
    ...
    public $SayfaBaslik = 'Bilgi Bankası';
    public  $modulName = 'Bilgi';
    public $icbaslik ;
    private $js = array();
    private $css = array();
    private $module = 'bilgi';
    private $table = 'bilgi';
    private $tablelang = 'bilgi_lang';
   ...
   //     $text .= $form->input(array('value'=>((isset($haber['baslik']) ? $haber['baslik'] :'')),'title'=>'Yorum Başlıgı','name'=>'baslik','id'=>'baslik','help'=>''));
        $text .= $form->input(array('value'=>((isset($haber['baslik']) ? $haber['baslik'] :'')),'title'=>'Başlık','name'=>'baslik','id'=>'baslik','help'=>''));
        $text .= $form->textEditor(array('value'=>((isset($haber['detay']) ? $this->temizle($haber['detay']) :'')),'title'=>'Açıklama','name'=>'detay','id'=>'detay','help'=>'','height'=>'320'));
  ...
  //      $text .= $form->file(array('url'=>$this->BaseURL().'upload','title'=>'Resim','name'=>'YorumResim','resimBoyut'=>$this->settings['haberResimBoyut'],'src'=>((isset($haber['resim'])) ? $haber['resim'] :'')));
    ...
    public function liste($id=null)
    {
      ...
      return $pagelist->Pagelist(array(
         'title'=> 'Bilgi Listesi',
         'button' => array(array('title'=>'Bilgi Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle.html'),'color'=>'green')),
         'p'=>array(
